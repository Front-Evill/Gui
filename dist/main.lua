...
function Section:AddButton(config)
	...
	local hasIcon = config.Icon and config.Icon ~= false
	local iconId = hasIcon and ResolveIcon((config.Icon == true) and "mouse-pointer" or config.Icon) or nil
	...
	if hasDescription then
		title.Size = UDim2.new(1, -textOffset, 0, 16)
...
function Section:AddCodeBlock(config)
	...
	if hasDescription then
		descLabel = CreateInstance("TextLabel", {
			...
			BackgroundTransparency = 1,
			Position = UDim2.new(0, 0, 0, textY),
			Size = UDim2.new(1, -titleReserve, 0, 13),
			Font = Enum.Font.Gotham,
			Text = config.Description,
...
function Section:AddLinks(config)
	...
	if hasDescription then
		descLabel = CreateInstance("TextLabel", {
			...
			Size = UDim2.new(textWidth.Scale, textWidth.Offset, 0, DESC_HEIGHT),
			Font = Enum.Font.Gotham,
			Text = config.Description,
			TextColor3 = FIXED_DESC_COLOR,
			TextSize = 11,
