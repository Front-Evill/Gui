...
rounded-corner look, unchanged).

```lua
icno = { work = true, Shape = "Circle" },
```
...
`IdIcon` — shows a triangle glyph in the middle instead of the small dot,
so it still reads clearly as "triangle" at a glance. If you do set your own
`IdIcon`, `Shape` only affects the corner rounding and your icon displays
