...
```lua
Section:AddBoxInfo({
	Title = "Player Info", -- optional, see below
	-- Country = "Iraq", -- optional manual override; otherwise auto-detected
...
the country on the line below. Everything is filled in automatically —
display name and username come straight from the player, join date is
worked out from `AccountAge`, and country is guessed from `LocaleId`
(override it with `Country` if you want something specific). Each piece can
be turned off individually with its own `Show...` flag, for example
`ShowUsername = false` to only show the display name and country.
