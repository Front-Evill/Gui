...
local AttachImageTooltip = Utils.AttachImageTooltip
...
local FIXED_GRAY = Color3.fromRGB(140, 140, 140)

local function DetectPublicIP(window, callback)
	if window._detectedIP then
		callback(window._detectedIP)
		return
	end

	if window._detectedIPWaiters then
		table.insert(window._detectedIPWaiters, callback)
		return
	end

	window._detectedIPWaiters = { callback }
	...
	task.spawn(function()
		...
		if ok and result then
			...
			if decodeOk and data and data.success ~= false and data.ip and data.ip ~= "" then
				ip = data.ip
			end
		end

		ip = ip or "Unknown"
		window._detectedIP = ip

		local waiters = window._detectedIPWaiters
		window._detectedIPWaiters = nil
...
local function SyncPairHeights(rowA, contentA, rowB, contentB)
	local MIN_CONTENT = 22
	local PADDING = 20

	local spacerA = CreateInstance("Frame", {
		Name = "PairSpacer",
		Parent = rowA,
		BackgroundTransparency = 1,
	...
	local spacerB = CreateInstance("Frame", {
		Name = "PairSpacer",
		Parent = rowB,
		BackgroundTransparency = 1,
		Size = UDim2.new(0, 0, 0, 0),
	...
	local function sync()
		local naturalA = math.max(contentA.AbsoluteSize.Y, MIN_CONTENT) + PADDING
		local naturalB = math.max(contentB.AbsoluteSize.Y, MIN_CONTENT) + PADDING
		local target = math.max(naturalA, naturalB)

		spacerA.Size = UDim2.new(0, 0, 0, math.max(0, target - naturalA))
		spacerB.Size = UDim2.new(0, 0, 0, math.max(0, target - naturalB))
	end

	contentA:GetPropertyChangedSignal("AbsoluteSize"):Connect(sync)
	contentB:GetPropertyChangedSignal("AbsoluteSize"):Connect(sync)
	sync()
end
...
local function ResolvePairSlot(self, elementType)
	...
	if self._lastPairRow and self._lastPairInstance and self._lastPairRow.Parent then
		...
		local previousContent = self._lastPairContent
		...
		self._lastPairContent = nil
		self._lastPairType = nil

		return parent, UDim2.new(0.5, -4, 0, 0), order, true, { Row = previousInstance, Content = previousContent }
	end
	...
	return pairRow, UDim2.new(1, 0, 0, 0), 1, false, nil
end

local function RememberPairSlot(self, parent, instance, content, elementType)
	self._lastPairRow = parent
	self._lastPairInstance = instance
	self._lastPairContent = content
...
function Section:AddButton(config)
	...
	local parent, rowSize, layoutOrder, isPaired, pairedWith = ResolvePairSlot(self, "Button")
	...
	if isPaired then
		SyncPairHeights(pairedWith.Row, pairedWith.Content, row, textBlock)
	else
		RememberPairSlot(self, parent, row, textBlock, "Button")
	end
...
function Section:AddToggle(config)
	...
	local parent, rowSize, layoutOrder, isPaired, pairedWith = ResolvePairSlot(self, "Toggle")
	...
	if isPaired then
		SyncPairHeights(pairedWith.Row, pairedWith.Content, row, textContainer)
	else
		RememberPairSlot(self, parent, row, textContainer, "Toggle")
	end
...
function Section:AddToggleGroup(config)
	config = config or {}
	self._lastPairRow = nil
	...
	local function setBodyOpen(open, instant)
		...
		if open then
			...
			if animated then
				...
				PlayTween(self.Window, body, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { GroupTransparency = 0 })
			else
	...
	local function setValue(newValue, fireCallback)
		...
		if fireCallback and config.Callback then
			local ok, err = pcall(config.Callback, value)
			if not ok then
				warn("Devlyx ToggleGroup Callback Error: " .. tostring(err))
			end
		end
	end
	...
	groupObject.Set = function(_, newValue)
		setValue(newValue, false)
	end

	groupObject.Get = function(_)
		return value
	end

	if config.Flag then
		self.Window.Flags[config.Flag] = groupObject
	end
...
function Section:_BuildDropdown(config, isMulti)
	...
	local optionsContainer = CreateInstance("CanvasGroup", {
		Name = "Options",
		Parent = row,
		BackgroundTransparency = 1,
	...
	local OPTION_ROW_GAP = 4
	local MAX_OPTIONS_LIST_HEIGHT = 190
	...
	table.insert(self.Window.ScrollFrames, optionsList)

	CreateInstance("UIListLayout", {
		Parent = optionsList,
		Padding = UDim.new(0, OPTION_ROW_GAP),
		SortOrder = Enum.SortOrder.LayoutOrder,
	})
	...
	local function setOpen(open)
		...
		if open then
			...
			optionsContainer.GroupTransparency = 1
			PlayTween(self.Window, optionsContainer, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { GroupTransparency = 0 })
		else
			PlayTween(self.Window, optionsContainer, TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { GroupTransparency = 1 })
	...
	local function isOptionSelected(optionName)
		if isMulti then
			return selectedSet[optionName] == true
		end
		return selectedSingle == optionName
	end

	local function applyOptionState(entry, selected, animate)
		local currentTheme = self.Window.Theme
		local bgColor = selected and currentTheme.Accent or currentTheme.Secondary
		local bgTransparency = selected and 0.82 or 0
		local strokeTransparency = selected and 0.35 or 1
		local labelColor = selected and currentTheme.Accent or currentTheme.TitleColor
		local checkTransparency = selected and 0 or 1

		if animate then
			local tweenInfo = TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
			PlayTween(self.Window, entry.Frame, tweenInfo, { BackgroundColor3 = bgColor, BackgroundTransparency = bgTransparency })
			PlayTween(self.Window, entry.Stroke, tweenInfo, { Transparency = strokeTransparency })
			PlayTween(self.Window, entry.Label, tweenInfo, { TextColor3 = labelColor })
			if entry.Check then
				PlayTween(self.Window, entry.Check, tweenInfo, { ImageTransparency = checkTransparency })
			end
		else
			entry.Frame.BackgroundColor3 = bgColor
			entry.Frame.BackgroundTransparency = bgTransparency
			entry.Stroke.Transparency = strokeTransparency
			entry.Label.TextColor3 = labelColor
			if entry.Check then
				entry.Check.ImageTransparency = checkTransparency
			end
		end
	end
	...
	local function rebuildOptions()
		...
		local naturalHeight = (#options * OPTION_ROW_HEIGHT) + math.max(0, #options - 1) * OPTION_ROW_GAP
		optionsList.Size = UDim2.new(1, 0, 0, math.min(naturalHeight, MAX_OPTIONS_LIST_HEIGHT))

		for _, optionName in ipairs(options) do
			local selected = isOptionSelected(optionName)
			...
			local optionRow = CreateInstance("TextButton", {
				...
				Text = "",
				AutoButtonColor = false,
				LayoutOrder = #optionsList:GetChildren(),
			})
			...
			local optionStroke = CreateInstance("UIStroke", {
				Parent = optionRow,
				Color = theme.Accent,
				Thickness = 1,
				Transparency = 1,
			...
			local entry = { Frame = optionRow, Label = optionLabel, Check = checkIcon, Stroke = optionStroke, Name = optionName }
			applyOptionState(entry, selected, false)

			optionRow.MouseEnter:Connect(function()
				if not isOptionSelected(optionName) then
					local hoverTween = TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
					PlayTween(self.Window, optionRow, hoverTween, {
						BackgroundColor3 = self.Window.Theme.Accent,
						BackgroundTransparency = 0.88,
					})
					PlayTween(self.Window, optionStroke, hoverTween, { Transparency = 0.6 })
				end
			end)
			...
			optionRow.MouseLeave:Connect(function()
				if not isOptionSelected(optionName) then
					...
					PlayTween(self.Window, optionStroke, hoverTween, { Transparency = 1 })
				end
			end)
			...
			optionRow.MouseButton1Click:Connect(function()
				if isMulti then
					...
					applyOptionState(entry, selectedSet[optionName], true)
					updateDisplay()
					...
					if selectedSingle ~= optionName then
						local previous = selectedSingle
						selectedSingle = optionName

						for _, other in ipairs(optionRows) do
							if other.Name == previous then
								applyOptionState(other, false, true)
							end
						end

						applyOptionState(entry, true, true)
					end
	...
	table.insert(self.Window.Controls, function(theme2)
		...
		for _, opt in ipairs(optionRows) do
			applyOptionState(opt, isOptionSelected(opt.Name), false)
		end
	end)
...
function Section:AddBoxInfo(config)
	...
	local showJoined = config.ShowJoined ~= false
	local showCountPlayers = config.ShowPlayerCount ~= false and config.ShowCountPlayers ~= false
	local showServerId = config.ShowServerId ~= false
	local showNameGame = config.ShowGameName ~= false and config.ShowNameGame ~= false
	local showIP = config.ShowIP ~= false
	...
	local columnsRow = CreateInstance("Frame", {
		Name = "Columns",
		Parent = topRow,
		BackgroundTransparency = 1,
		Position = UDim2.new(0, showAvatar and (avatarSize + 12) or 0, 0, 0),
		Size = UDim2.new(1, showAvatar and -(avatarSize + 12) or 0, 0, 0),
		AutomaticSize = Enum.AutomaticSize.Y,
	})
	...
	local columnLeft = CreateInstance("Frame", {
		Name = "ColumnLeft",
		Parent = columnsRow,
		BackgroundTransparency = 1,
	...
	local columnRight = CreateInstance("Frame", {
		Name = "ColumnRight",
		Parent = columnsRow,
		BackgroundTransparency = 1,
		Size = UDim2.new(0.5, -8, 0, 0),
	...
	local function addInfoLine(parent, order, name)
	...
	local usernameLabel, nameLabel, joinedLabel
	if showUsername then
		usernameLabel = addInfoLine(columnLeft, 1, "Username")
		usernameLabel.Text = "Username: " .. ((player and player.Name) or "unknown")
	end
	if showName then
		nameLabel = addInfoLine(columnLeft, 2, "PlayerName")
		nameLabel.Text = "Name: " .. ((player and player.DisplayName) or "Player")
	end
	if showJoined then
		joinedLabel = addInfoLine(columnLeft, 3, "Joined")
		joinedLabel.Text = "Joined: " .. FormatJoinDate(player)
	end

	local countPlayersLabel, serverIdLabel, nameGameLabel

	local function currentCountPlayersText()
	...
	local function currentServerIdText()
		local ok, text = pcall(function()
			local id = game.JobId
	...
	local function currentNameGameText()
		local ok, text = pcall(function()
			return game.Name
		end)
		return (ok and text and text ~= "") and text or "Unknown"
	end

	if showCountPlayers then
		countPlayersLabel = addInfoLine(columnRight, 1, "PlayerCount")
		countPlayersLabel.Text = "Players: " .. currentCountPlayersText()
	end
	if showServerId then
		serverIdLabel = addInfoLine(columnRight, 2, "ServerId")
		serverIdLabel.Text = "ServerId: " .. currentServerIdText()
	end
	if showNameGame then
		nameGameLabel = addInfoLine(columnRight, 3, "GameName")
		nameGameLabel.Text = "Game: " .. currentNameGameText()
	end

	local ipLabel
	if showIP then
		local manualIP = config.IP and config.IP ~= "" and config.IP or nil
		local initialText = manualIP or self.Window._detectedIP or "Detecting..."

		ipLabel = CreateInstance("TextLabel", {
			Name = "IP",
			Parent = card,
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, 0),
			...
			Font = Enum.Font.Gotham,
			Text = "ip: " .. initialText,
			TextColor3 = FIXED_GRAY,
			TextSize = 10,
			TextXAlignment = Enum.TextXAlignment.Center,
			LayoutOrder = 2,
		})

		if not manualIP then
			DetectPublicIP(self.Window, function(detected)
				if ipLabel and ipLabel.Parent then
					ipLabel.Text = "ip: " .. detected
				end
			end)
		end
	end
	...
	local boxInfoObject = {
		...
		Refresh = function()
			...
			if usernameLabel then
				usernameLabel.Text = "Username: " .. ((currentPlayer and currentPlayer.Name) or "unknown")
			end
			if nameLabel then
				nameLabel.Text = "Name: " .. ((currentPlayer and currentPlayer.DisplayName) or "Player")
			end
			if joinedLabel then
				joinedLabel.Text = "Joined: " .. FormatJoinDate(currentPlayer)
			end
			if countPlayersLabel then
				countPlayersLabel.Text = "Players: " .. currentCountPlayersText()
			end
			if serverIdLabel then
				serverIdLabel.Text = "ServerId: " .. currentServerIdText()
			end
			if nameGameLabel then
				nameGameLabel.Text = "Game: " .. currentNameGameText()
			end
			if ipLabel and (not config.IP or config.IP == "") then
				ipLabel.Text = "ip: " .. (self.Window._detectedIP or "Detecting...")
			end
...
local function ResolveImagePairSlot(self)
	local children = self.Container:GetChildren()
	local layoutOrder = #children

	if self._lastImageRow and self._lastImageRow.Parent and self._lastImageCount == 1 then
		local lastChild = children[#children]
		if lastChild == self._lastImageRow then
			self._lastImageCount = 2
			local row = self._lastImageRow
			self._lastImageRow = nil
			self._lastImageCount = nil
			return row
		end
	end
	...
	self._lastImageRow = row
	self._lastImageCount = 1
...
function Section:AddImage(config)
	...
	local theme = self.Window.Theme
	local size = 120
	local imageSource = config.Image or config.Icon or ""
	local callback = config.Callback

	local row = ResolveImagePairSlot(self)
	...
	local hoverScale = CreateInstance("UIScale", {
		Parent = imageButton,
		Scale = 1,
	})

	local picture = CreateInstance("ImageLabel", {
		Name = "Picture",
		Parent = imageButton,
		BackgroundTransparency = 1,
		Size = UDim2.fromScale(1, 1),
		Image = ResolveIcon(imageSource),
	...
	local hoverTween = TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
	local leaveTween = TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

	imageButton.MouseEnter:Connect(function()
		PlayTween(self.Window, hoverScale, hoverTween, { Scale = 1.08 })
		PlayTween(self.Window, stroke, hoverTween, { Transparency = 0.1 })
	end)

	imageButton.MouseLeave:Connect(function()
		PlayTween(self.Window, hoverScale, leaveTween, { Scale = 1 })
		PlayTween(self.Window, stroke, leaveTween, { Transparency = 0.4 })
	end)
	...
	if callback then
		imageButton.MouseButton1Click:Connect(function()
			...
			if not ok then
				warn("Devlyx Image Callback Error: " .. tostring(err))
			end
		end)
	end

	AttachImageTooltip(self.Window, imageButton, config.Title, config.Description)

	table.insert(self.Window.Controls, function(theme2)
		imageButton.BackgroundColor3 = theme2.Background
		imageButton.BackgroundTransparency = theme2.Transparency
		stroke.Color = theme2.Accent
	end)
	...
	return {
		...
		SetImage = function(_, newImage)
			picture.Image = ResolveIcon(newImage)
		end,
	}
end
