...
local function DetectCountryFromIP(window, player, callback)
	if window._detectedCountry then
		callback(window._detectedCountry)
		return
	end

	if window._detectedCountryWaiters then
		table.insert(window._detectedCountryWaiters, callback)
		return
	end

	window._detectedCountryWaiters = { callback }
	...
	task.spawn(function()
		...
		if ok and result then
			...
			if decodeOk and data and data.success ~= false and data.country and data.country ~= "" then
				country = data.country
			end
		end

		if not country then
			country = ResolveCountryFromLocale(player)
		end

		window._detectedCountry = country

		local waiters = window._detectedCountryWaiters
		window._detectedCountryWaiters = nil
...
function Section:AddBoxInfo(config)
	...
	if showCountry then
		local manualCountry = config.Country and config.Country ~= "" and config.Country or nil
		local initialText = manualCountry or self.Window._detectedCountry or "Detecting..."
		...
		if not manualCountry then
			DetectCountryFromIP(self.Window, player, function(detected)
				if countryLabel and countryLabel.Parent then
					countryLabel.Text = detected
				end
			end)
		end
	end
	...
	local boxInfoObject = {
		...
		Refresh = function()
			...
			if countryLabel and (not config.Country or config.Country == "") then
				countryLabel.Text = self.Window._detectedCountry or ResolveCountryFromLocale(currentPlayer)
			end
