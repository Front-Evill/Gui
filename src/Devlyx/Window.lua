...
function M:Window(config)
	...
	local function updateResizeHandlesVisibility()
		for _, h in ipairs(resizeHandles) do
			h.Visible = not isCollapsed and not isMaximized
		end
	end
	...
	local function setMaximized(maximized)
		...
		local targetSize
		if maximized then
			storedWindowSize = mainFrame.Size
			local camera = Workspace.CurrentCamera
	...
	local iconConfig = config.Icon or config.icno
	if iconConfig and iconConfig.work then
		local iconImageValue = iconConfig.Image or iconConfig.IdIcon
		local hasImage = iconImageValue and iconImageValue ~= ""
		local iconSize = iconConfig.Size or 44
		local innerImageSize = math.floor(iconSize * 0.9)
		local innerCornerSize = math.floor(iconSize * 0.45)

		local shape = iconConfig.Shape
		...
		if hasImage then
			iconImageLabel = CreateInstance("ImageLabel", {
				...
				BackgroundTransparency = 1,
				Image = ResolveIcon(iconImageValue),
			})
...
function M:SetTheme(newTheme)
	...
	for _, section in ipairs(self.SectionLabels) do
		if section and section.Parent then
			...
			table.insert(aliveSectionLabels, section)
		end
	end
	self.SectionLabels = aliveSectionLabels
	...
	for _, refresh in ipairs(self.Controls) do
		local ok = pcall(refresh, self.Theme)
		if ok then
			table.insert(aliveControls, refresh)
		end
	end
	self.Controls = aliveControls

	local aliveScrollFrames = {}
	for _, scrollFrame in ipairs(self.ScrollFrames) do
		if scrollFrame and scrollFrame.Parent then
			...
			table.insert(aliveScrollFrames, scrollFrame)
		end
	end
	self.ScrollFrames = aliveScrollFrames
