...
local function EnsureBoxSystem(self)
	...
	for _, child in ipairs(elementsRoot:GetChildren()) do
...
local function EnsureSubTabSystem(self)
	...
	local barDivider = CreateInstance("Frame", {
		Name = "Divider",
		Parent = stageFrame,
	...
	table.insert(self.Window.Controls, function(theme2)
		...
		barDivider.BackgroundColor3 = theme2.Accent
	end)
...
function Tab:AddSubTab(config)
	...
	local function ApplyButtonStyle(animate)
		...
		if animate then
			...
			PlayTween(self.Window, underline, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
				BackgroundTransparency = underlineTransparency,
				Size = underlineSize,
			})
			PlayTween(self.Window, scale, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
				Scale = targetScale,
			})
			if iconImage then
