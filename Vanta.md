...
Section:AddButton({
	...
	Icon = true, -- shows a mouse-pointer icon on the left; pass an icon name instead for a custom one
	Callback = function()
		print("deleted")
	end,
	--[[
...
`Icon` is optional: leave it out for a plain text button, set it to `true`
for a small mouse-pointer icon on the left, or give it any Lucide icon name
(or asset id) for a custom one.
